...
public class MessyExpressions {
    ...
    public static boolean isJustScreaming(String text) {
        ...
        String removeA = upperCaseText.replace("A", "");
        String removeExclamation = removeA.replace("!", "");
        return removeExclamation.isBlank();
    }
    ...
    public static boolean containsOwnLength(String text) {
        if (text.contains("0")) {
            int len = text.length();
            String newlen = String.valueOf(len);
            return text.contains(newlen);
        }
        if (text.contains("1")) {
            int len = text.length();
            String newlen = String.valueOf(len);
